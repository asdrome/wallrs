...
## 🎯 Fase 1: Quick Wins & Ajustes Autocontenidos (✅ Completada - Commit `0946098`)
...
## 🚀 Fase 2: Infraestructura Wayland & Desacoplamiento CLI (✅ Completada - Commits `43cd961` y `d7b7285`)
...
## 🏗️ Fase 3: Refactor Estructural e Inversión de Control (✅ Completada - Commit `1c4cb57`)
...
## 🎥 Fase 4: Migración Integral a GStreamer (✅ Completada - Commit `cb809b6`)
...
## 📊 Matriz de Prioridad, Viabilidad y Estado de Entrega

|  Fase   | Tarea                        |   Viabilidad    |      Autocontención      |  Impacto en Usuario   |    Estado    |
| :-----: | :--------------------------- | :-------------: | :----------------------: | :-------------------: | :----------: |
| **1.1** | Mapeo de uniforms por nombre |   🟢 Muy Alta    | 🟢 Alta (local en shader) |      🟢 Inmediato      | ✅ Completado |
| **1.2** | FPS ceiling para shaders     |   🟢 Muy Alta    | 🟢 Alta (proto + shader)  |     🟢 Ahorro GPU      | ✅ Completado |
| **1.3** | Parser unificado de color    |   🟢 Muy Alta    |  🟢 Alta (utilidad pura)  |    🟡 Consistencia     | ✅ Completado |
| **1.4** | Mute/Unmute nativo en IPC    |   🟢 Muy Alta    |    🟢 Alta (protocolo)    |   🟢 Menor latencia    | ✅ Completado |
| **2.1** | Wayland HiDPI Scaling        |     🟡 Media     |   🟡 Media (motor WGPU)   |   🟢 Calidad visual    | ✅ Completado |
| **2.2** | Desacoplamiento de CLI       |     🟡 Media     |  🟡 Media (dependencias)  |   🟡 Binario ligero    | ✅ Completado |
| **2.3** | Consolidación de Audio       |     🟡 Media     |      🟡 Media (core)      |   🟡 Mantenibilidad    | ✅ Completado |
| **3.1** | Registry de Plugins (DIP)    |     🟡 Media     |   🔴 Estructural (core)   | 🟢 Arquitectura limpia | ✅ Completado |
| **4.1** | GStreamer appsink básico     |     🟡 Media     |     🟡 Local en video     |  🟢 Estabilidad Linux  | ✅ Completado |
| **4.2** | Zero-Copy DMA-BUF            | 🔴 Baja/Compleja |  🔴 Requiere HW testing   |  🟢 Máxima eficiencia  | ✅ Completado |
| **4.3** | GStreamer Ambient Audio      |     🟢 Alta      |     🟢 Local en audio     |   🟢 PipeWire nativo   | ✅ Completado |
| **4.4** | Remoción total de libmpv     |     🟢 Alta      |      🟡 Empaquetado       |    🟢 Portabilidad     | ✅ Completado |

---

## 🔍 Observaciones de Auditoría Post-1.2 (En Progreso)

> [!WARNING]
> **Criterio**: Ajustes críticos identificados durante la auditoría de la migración a GStreamer para evitar cuellos de botella y crashes.

### 5.1 Inyección Inteligente de Filtros de Hardware (`vapostproc`) (✅ Completada)
* **Crate**: `wallrs-content-video`
* **Implementación**:
  * Eliminada la inyección estática e incondicional de `vapostproc` al configurar `playbin`.
  * Implementada la función `is_vaapi_decoder` para inspeccionar el decodificador activo dentro del callback `element-setup`.
  * Implementada la sonda `is_vapostproc_usable` para verificar la inicialización real de `/dev/dri/renderD128` antes de adjuntar el filtro.
  * Inyección dinámica de `vapostproc` como `video-filter` activada exclusivamente cuando el decodificador es VA-API. En decodificadores NVIDIA (`nvdec`), software (`avdec`) o entornos sin VA-API, el pipeline fluye directo hacia `appsink`, evitando fallas de negociación, saturación de PCIe y crashes.

### 5.2 Implementación del Auténtico Zero-Copy DMA-BUF
* **Crate**: `wallrs-content-video`
* **Objetivo**:
  * Aunque se redujo el ancho de banda PCIe forzando NV12 (1.5 bytes/pixel en vez de RGBA 4 bytes), actualmente GStreamer sigue bajando el frame de memoria GPU a memoria RAM (CPU) para que wgpu lo vuelva a subir a textura (`ctx.queue.write_texture`).
  * Investigar e implementar el soporte nativo de exportación DMA-BUF de GStreamer e importación en WGPU usando EGLImage para lograr verdaderamente un pipeline *Zero-Copy* (GPU a GPU).
