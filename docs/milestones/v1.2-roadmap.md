...
### 5.1 Inyección Inteligente de Filtros de Hardware (vapostproc)
* **Crate**: `wallrs-content-video`
* **Objetivo**:
  * Actualmente el pipeline inyecta ciegamente `vapostproc` si está disponible en el sistema. Esto rompe la reproducción o destroza el rendimiento si se usan decodificadores NVIDIA (nvdec) o de software.
  * Modificar la lógica para inyectar `vapostproc` *solo* si se verifica que el decodificador activo está utilizando `va-api` (o delegar la responsabilidad a bins autoconfigurables).
